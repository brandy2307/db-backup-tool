...
class DatabaseBackupTool {
  constructor() {
    ...
    console.log(
      `   Max Login-Versuche: ${this.securityConfig.maxFailedAttempts}`
    );
    console.log(
      `   CAPTCHA nach: ${this.securityConfig.captchaThreshold} Fehlversuchen`
    );
    console.log(
      `   Session Timeout: ${
        this.securityConfig.sessionTimeout / 1000 / 60
      } Minuten`
    );
  ...
  loadConfig() {
    try {
      ...
      // ====== BOOLEAN UMGEBUNGSVARIABLEN KORREKT VERARBEITEN ======
      ...
      if (process.env.GIT_BACKUP_ENABLED !== undefined) {
      ...
      // ====== SICHERHEITS-KONFIGURATION KORREKTE BOOLEAN-BEHANDLUNG ======
      if (process.env.REQUIRE_HTTPS !== undefined) {
        config.security = config.security || {};
        config.security.requireHttps = process.env.REQUIRE_HTTPS === "true";
        console.log(
          `🔐 [CONFIG] REQUIRE_HTTPS: ${process.env.REQUIRE_HTTPS} -> ${config.security.requireHttps}`
        );
      }
      if (process.env.ENABLE_2FA !== undefined) {
        config.security = config.security || {};
        config.security.enable2FA = process.env.ENABLE_2FA === "true";
        console.log(
          `🔐 [CONFIG] ENABLE_2FA: ${process.env.ENABLE_2FA} -> ${config.security.enable2FA}`
        );
      }
      if (process.env.STRONG_PASSWORDS !== undefined) {
        config.security = config.security || {};
        config.security.strongPasswords =
          process.env.STRONG_PASSWORDS === "true";
        console.log(
          `🔐 [CONFIG] STRONG_PASSWORDS: ${process.env.STRONG_PASSWORDS} -> ${config.security.strongPasswords}`
        );
      }
      ...
      // ====== SSL-KONFIGURATION KORREKTE BOOLEAN-BEHANDLUNG ======
      ...
      if (process.env.SSL_AUTO_RENEWAL !== undefined) {
        config.ssl = config.ssl || {};
        config.ssl.autoRenewal = process.env.SSL_AUTO_RENEWAL === "true";
        console.log(
          `🔐 [CONFIG] SSL_AUTO_RENEWAL: ${process.env.SSL_AUTO_RENEWAL} -> ${config.ssl.autoRenewal}`
        );
      }
      ...
      if (process.env.SSL_SETUP_ON_START !== undefined) {
        config.ssl = config.ssl || {};
        config.ssl.setupOnStart = process.env.SSL_SETUP_ON_START === "true";
        console.log(
          `🔐 [CONFIG] SSL_SETUP_ON_START: ${process.env.SSL_SETUP_ON_START} -> ${config.ssl.setupOnStart}`
        );
      }
      ...
      console.log("🔧 [CONFIG DEBUG] Finaler Konfigurationsstatus:");
      console.log(
        `   requireHttps: ${
          config.security?.requireHttps
        } (Type: ${typeof config.security?.requireHttps})`
      );
      console.log(
        `   enable2FA: ${config.security?.enable2FA} (Type: ${typeof config
          .security?.enable2FA})`
      );
      console.log(
        `   strongPasswords: ${
          config.security?.strongPasswords
        } (Type: ${typeof config.security?.strongPasswords})`
      );
      console.log(
        `   SSL setupOnStart: ${config.ssl?.setupOnStart} (Type: ${typeof config
          .ssl?.setupOnStart})`
      );
      console.log(
        `   SSL autoRenewal: ${config.ssl?.autoRenewal} (Type: ${typeof config
          .ssl?.autoRenewal})`
      );
...
`;
  ...
  async runSSLSetup(sslConfig) {
    return new Promise((resolve, reject) => {
      ...
      exec(
        `chmod +x "${sslSetupPath}" && "${sslSetupPath}"`,
        {
          env: env,
          cwd: __dirname,
  ...
  validatePasswordStrength(password) {
    ...
    if (errors.length > 0) {
      return {
        ...
        strength:
          errors.length > 3 ? "weak" : errors.length > 1 ? "medium" : "strong",
      };
    }
  ...
  validateCaptcha(captchaId, userInput) {
    ...
    if (!captchaSession) {
      console.log(
        `❌ [CAPTCHA] Session nicht gefunden oder abgelaufen: ${captchaId}`
      );
    ...
    if (userInput.toLowerCase() !== captchaSession.text) {
      console.log(
        `❌ [CAPTCHA] Falsche Eingabe: ${userInput} !== ${captchaSession.text}`
      );
  ...
  recordFailedAttempt(ip, username) {
    ...
    if (attempt.count >= this.securityConfig.maxFailedAttempts) {
      ...
      console.log(
        `🔒 [SECURITY] Account gesperrt: ${username} von ${ip} für ${
          this.securityConfig.lockoutDuration / 1000 / 60
        } Minuten`
      );
    }

    console.log(
      `⚠️ [SECURITY] Failed attempt #${attempt.count} für ${username} von ${ip}`
    );
  }
  ...
  clearFailedAttempts(ip, username) {
    ...
    console.log(
      `✅ [SECURITY] Failed attempts gelöscht für ${username} von ${ip}`
    );
  }
  ...
  createSecureSession(req, user, rememberMe = false) {
    ...
    console.log(
      `🔐 [SESSION] Neue Session erstellt: ${sessionId} für ${user.username}`
    );
    return sessionId;
  }
  ...
  validateSession(sessionId, req) {
    ...
    if (session.ip !== req.ip && !session.rememberMe) {
      console.log(
        `⚠️ [SESSION] IP-Änderung erkannt: ${session.ip} -> ${req.ip} für Session ${sessionId}`
      );
    ...
    const totalTime = session.rememberMe
      ? 7 * 24 * 60 * 60 * 1000
      : this.securityConfig.sessionTimeout;
  ...
  invalidateSession(sessionId) {
    ...
    if (session) {
      this.activeSessions.delete(sessionId);
      console.log(
        `🔐 [SESSION] Session invalidiert: ${sessionId} für ${session.userId}`
      );
    }
  }
  ...
  authMiddleware(req, res, next) {
    ...
    try {
      ...
      console.log(
        `❌ [AUTH] Token-Validierung fehlgeschlagen: ${error.message}`
      );
  ...
  async setupDefaultUser() {
    ...
    if (this.securityConfig.strongPasswords) {
      ...
      if (!validation.valid) {
        console.warn(
          `⚠️ [SECURITY] Standard-Admin-Passwort erfüllt nicht die Sicherheitsanforderungen:`
        );
    ...
    try {
      ...
      if (adminPassword === "admin123" || adminPassword === "admin") {
        console.warn(
          `⚠️ [SECURITY] Verwende Standard-Passwort! Ändere es nach dem ersten Login.`
        );
      }
    } catch (error) {
      console.error(
        "❌ [USER] Fehler beim Erstellen des Standard-Admin:",
        error
      );
      throw error;
    }
  ...
  setupMiddleware() {
      ...
      cors({
        origin: (origin, callback) => {
          ...
          const allowedOrigins = [
            ...
            "http://localhost:8080",
            `https://${this.config.ssl?.domain}:${
              this.config.server.httpsPort || 8443
            }`,
    ...
    this.app.use((req, res, next) => {
      ...
      res.send = function (data) {
        ...
        console.log(
          `${statusIcon} [${logLevel}] ${req.method} ${urlToLog} - ${res.statusCode} - ${duration}ms - ${req.ip}`
        );

        // Sicherheits-Events separat loggen
        if (req.url.includes("/api/login") || req.url.includes("/api/logout")) {
          console.log(
            `🔐 [AUTH] ${req.method} ${req.url} - ${res.statusCode} - ${
              req.ip
    ...
    this.app.use((req, res, next) => {
      ...
      if (req.url.includes("/admin/") && process.env.ADMIN_WHITELIST_IPS) {
        const whitelistedIPs = process.env.ADMIN_WHITELIST_IPS.split(",").map(
          (ip) => ip.trim()
        );
        if (
          !whitelistedIPs.includes(clientIP) &&
          !whitelistedIPs.includes("0.0.0.0")
        ) {
          console.log(
            `🚫 [SECURITY] Non-whitelisted IP blocked from admin area: ${clientIP}`
          );
          return res.status(403).json({
      ...
      if (["POST", "PUT", "DELETE", "PATCH"].includes(req.method)) {
        ...
        if (req.headers.authorization || req.cookies["auth-token"]) {
          if (!token || !sessionToken || token !== sessionToken) {
            console.log(
              `❌ [CSRF] CSRF token mismatch: ${req.ip} - ${req.url}`
            );
            return res.status(403).json({
    ...
    this.app.use((err, req, res, next) => {
      console.error(
        `❌ [MIDDLEWARE ERROR] ${err.message} - ${req.method} ${req.url} - ${req.ip}`
      );
  ...
  ensureDirectories() {
    ...
    try {
      ...
      console.warn(
        `⚠️ [DIRECTORY] Konnte SSL-Verzeichnis-Berechtigungen nicht setzen: ${error.message}`
      );
    }
  ...
  async initializeGitBackup() {
    ...
    if (!this.config.gitBackup.repository || !this.config.gitBackup.username) {
      console.warn(
        "⚠️ [GIT BACKUP] Git Backup aktiviert aber Repository oder Username fehlt"
      );
      return;
    }
    ...
    try {
      ...
      if (!fs.existsSync(gitDir)) {
        ...
        console.log(
          "📝 [GIT BACKUP] Repository bereits vorhanden, prüfe Verbindung..."
        );
      ...
      console.error(
        "❌ [GIT BACKUP] Initialisierung fehlgeschlagen:",
        error.message
      );
  ...
  setupRoutes() {
      ...
      (req, res) => {
        try {
          ...
          qrcode.toDataURL(secret.qrCode, (err, qrCodeDataUrl) => {
            if (err) {
              console.error(
                "❌ [2FA] QR-Code Generierung fehlgeschlagen:",
                err
              );
              return res
                .status(500)
                .json({ error: "QR-Code Generierung fehlgeschlagen" });
            }
      ...
      (req, res) => {
        try {
          ...
          if (!user || !user.tempTwoFactorSecret) {
            return res
              .status(400)
              .json({ error: "Kein 2FA-Setup in Bearbeitung" });
          }
    ...
    this.app.post("/api/login", async (req, res) => {
      ...
      console.log(
        `🔐 [LOGIN] Versuch von ${clientIp} für Benutzer: ${username}`
      );
      ...
      try {
        ...
        if (lockStatus.locked) {
          console.log(
            `🔒 [LOGIN] Account gesperrt: ${sanitizedUsername} von ${clientIp}`
          );
        ...
        if (this.needsCaptcha(clientIp, sanitizedUsername)) {
          if (!captchaId || !captchaText) {
            console.log(
              `🤖 [LOGIN] CAPTCHA erforderlich für: ${sanitizedUsername}`
            );
            return res.status(400).json({
          ...
          if (!captchaValidation.valid) {
            ...
            console.log(
              `❌ [LOGIN] CAPTCHA fehlgeschlagen für: ${sanitizedUsername}`
            );
            return res.status(400).json({
        ...
        if (!user) {
          this.recordFailedAttempt(clientIp, sanitizedUsername);
          console.log(
            `❌ [LOGIN] Benutzer nicht gefunden: ${sanitizedUsername}`
          );
        ...
        if (this.securityConfig.enable2FA && user.twoFactorSecret) {
          if (!twoFactorToken) {
            console.log(
              `🔐 [LOGIN] 2FA-Token erforderlich für: ${sanitizedUsername}`
            );
            return res.status(400).json({
          ...
          if (!this.verify2FAToken(user.twoFactorSecret, twoFactorToken)) {
            this.recordFailedAttempt(clientIp, sanitizedUsername);
            console.log(
              `❌ [LOGIN] 2FA-Token ungültig für: ${sanitizedUsername}`
            );
            return res.status(401).json({
        ...
        console.log(
          `✅ [LOGIN] Erfolgreicher Login für ${sanitizedUsername} von ${clientIp}`
        );
        ...
        res.json({
          ...
          requiresPasswordChange:
            !user.passwordChanged && this.securityConfig.strongPasswords,
        });
      } catch (error) {
    ...
    this.app.post("/api/logout", (req, res) => {
      const token =
        req.headers.authorization?.split(" ")[1] || req.cookies["auth-token"];
      ...
      async (req, res) => {
        try {
          ...
          if (!currentPasswordValid) {
            console.log(
              `❌ [PASSWORD] Aktuelles Passwort ungültig für: ${username}`
            );
            return res
              .status(401)
              .json({ error: "Aktuelles Passwort ungültig" });
          }
          ...
          console.log(
            `🔐 [PASSWORD] Passwort geändert für Benutzer: ${username}`
          );
          ...
          console.error(
            "❌ [PASSWORD] Passwort-Änderung fehlgeschlagen:",
            error
          );
    ...
    this.app.get("/api/session-status", (req, res) => {
      ...
      try {
        ...
        console.log(
          `❌ [SESSION] Token-Validierung fehlgeschlagen: ${error.message}`
        );
        res.json({
          authenticated: false,
    ...
    console.log(
      "✅ [ROUTES] Sicherheits- und Authentifizierungs-Routen konfiguriert"
    );
    ...
    this.app.post(
      ...
      async (req, res) => {
        try {
          const { username } = req.user;
          console.log(
            `🔄 [UPDATE] Manuelles Update gestartet von: ${username}`
          );
    ...
    this.app.get(
      ...
      (req, res) => {
        ...
        console.log(
          `📋 [GIT CONFIG] Konfiguration abgerufen von: ${req.user.username}`
        );
        res.json(config);
      }
    );
    ...
    this.app.post(
      ...
      async (req, res) => {
        try {
          ...
          console.log(
            `📦 [BACKUP] Backup-Erstellung gestartet von: ${currentUser}`
          );
          ...
          res
            .status(500)
            .json({
              error: "Backup-Erstellung fehlgeschlagen: " + error.message,
            });
        }
      }
    );
      ...
      (req, res) => {
        try {
          ...
          if (!fs.existsSync(filePath)) {
            return res
              .status(404)
              .json({ error: "Backup-Datei nicht gefunden" });
          }
          ...
          if (stats.isDirectory()) {
            return res
              .status(400)
              .json({
                error: "Verzeichnis-Downloads werden nicht unterstützt",
              });
          }

          console.log(
            `📥 [DOWNLOAD] ${req.user.username} lädt herunter: ${filename}`
          );
      ...
      (req, res) => {
        try {
          ...
          if (req.user.role !== "admin") {
            ...
            console.log(
              `⚠️ [DELETE] Nicht-Admin versucht Backup zu löschen: ${req.user.username}`
            );
          }

          fs.unlinkSync(filePath);
          console.log(
            `🗑️ [DELETE] ${req.user.username} hat gelöscht: ${filename}`
          );
        ...
        try {
          ...
          const filteredSchedules =
            req.user.role === "admin"
              ? schedules
              : schedules.filter(
                  (schedule) => schedule.createdBy === req.user.username
                );

          res.json(filteredSchedules);
        } catch (error) {
          console.error(
            "❌ [SCHEDULE] Fehler beim Laden der Zeitpläne:",
            error
          );
        ...
        try {
          ...
          if (!name || !cronExpression || !dbConfig) {
            return res
              ...
              .json({
                error:
                  "Name, Cron-Expression und Datenbank-Konfiguration sind erforderlich",
              });
          }
          ...
          console.log(
            `📅 [SCHEDULE] Neuer Zeitplan erstellt: ${name} von ${username}`
          );
          ...
          console.error(
            "❌ [SCHEDULE] Fehler beim Erstellen des Zeitplans:",
            error
          );
    ...
    this.app.delete(
      ...
      (req, res) => {
        try {
          ...
          console.log(
            `🗑️ [SCHEDULE] Zeitplan gelöscht: ${schedule.name} von ${username}`
          );
          res.json({ message: "Zeitplan erfolgreich gelöscht" });
        } catch (error) {
          console.error(
            "❌ [SCHEDULE] Fehler beim Löschen des Zeitplans:",
            error
          );
    ...
    this.app.use((req, res) => {
      console.log(
        `❌ [404] Nicht gefundener Endpunkt: ${req.method} ${req.url} von ${req.ip}`
      );
      res.status(404).json({
    ...
    this.app.use((error, req, res, next) => {
      console.error(
        `❌ [ERROR] Unbehandelter Fehler: ${error.message} bei ${req.method} ${req.url}`
      );
      res.status(500).json({
        error: "Interner Serverfehler",
      ...
      async (req, res) => {
        try {
          if (!this.sslManager) {
            return res.json({
              enabled: false,
              reason: "SSL Manager nicht initialisiert",
            });
          }
          ...
          res
            ...
            .json({
              error:
                "SSL Status konnte nicht abgerufen werden: " + error.message,
            });
        }
      }
    );
      ...
      async (req, res) => {
        ...
        try {
          if (!this.sslManager) {
            return res
              .status(500)
              .json({ error: "SSL Manager nicht verfügbar" });
          }

          console.log(
            `🔄 [SSL] Manuelle Erneuerung durch: ${req.user.username}`
          );
          ...
          console.error(
            "❌ [SSL] Manuelle Erneuerung fehlgeschlagen:",
            error.message
          );
          res
            .status(500)
            .json({ error: "SSL-Erneuerung fehlgeschlagen: " + error.message });
        }
      }
    );
      ...
      async (req, res) => {
        try {
          if (!this.sslHealthCheck) {
            return res.json({
              overall: "UNKNOWN",
              reason: "SSL Health Check nicht verfügbar",
            });
          }

          const health =
            await this.sslHealthCheck.performComprehensiveHealthCheck();
          res.json(health);
        } catch (error) {
          ...
          res
            .status(500)
            .json({
              error: "SSL Health Check fehlgeschlagen: " + error.message,
            });
        }
      }
    );
    ...
    this.app.get(
      ...
      (req, res) => {
        try {
          ...
          console.error(
            "❌ [SSL] Konfiguration konnte nicht abgerufen werden:",
            error
          );
          res
            .status(500)
    ...
    this.app.get(
      ...
      (req, res) => {
        try {
          ...
          console.error(
            "❌ [SSL] Setup-Anweisungen konnten nicht generiert werden:",
            error
          );
          res
            .status(500)
    ...
    this.app.post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          res
            .status(500)
            .json({ error: "SSL Test fehlgeschlagen: " + error.message });
        }
      }
    );
    ...
    this.app.get(
      ...
      async (req, res) => {
        ...
        try {
          ...
          res
            .status(500)
            .json({ error: "SSL Export fehlgeschlagen: " + error.message });
        }
      }
    );
  ...
  loadGitToken() {
    try {
      if (fs.existsSync(this.secretsFile)) {
        ...
        console.log(
          `🔑 [GIT TOKEN] Token geladen (${secrets.token?.length || 0} Zeichen)`
        );
        return secrets.token;
      }
    } catch (error) {
      console.error(
        "❌ [GIT TOKEN] Fehler beim Laden des Tokens:",
        error.message
      );
    }
    return null;
  ...
  saveGitToken(token) {
    try {
      ...
      console.error(
        "❌ [GIT TOKEN] Fehler beim Speichern des Tokens:",
        error.message
      );
      throw error;
    }
  ...
  deleteGitToken() {
    try {
      ...
      console.error(
        "❌ [GIT TOKEN] Fehler beim Löschen des Tokens:",
        error.message
      );
    }
  }
  ...
  async updateGitBackupConfig(req, res) {
    try {
      ...
      console.log(
        `🔧 [GIT CONFIG] Konfiguration wird aktualisiert von: ${currentUser}`
      );
      ...
      if (enabled) {
        ...
        if (
          ...
          !repository.includes("bitbucket.org")
        ) {
          console.warn(
            `⚠️ [GIT CONFIG] Unbekannter Git-Provider: ${repository}`
          );
        }
      }
      ...
      if (enabled) {
        try {
          ...
          console.error(
            "❌ [GIT CONFIG] Git Backup Initialisierung fehlgeschlagen:",
            gitError.message
          );
          return res.status(500).json({
            error:
              "Git Backup Konfiguration gespeichert, aber Initialisierung fehlgeschlagen: " +
              gitError.message,
      ...
      console.error(
        "❌ [GIT CONFIG] Fehler beim Aktualisieren der Konfiguration:",
        error
      );
      res
        .status(500)
        .json({
          error: "Konfiguration-Update fehlgeschlagen: " + error.message,
        });
    }
  }
  ...
  async cloneGitBackupRepository() {
    if (
      !this.config.gitBackup?.repository ||
      !this.config.gitBackup?.username ||
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(command, { timeout: 60000 }, (error, stdout, stderr) => {
        if (error) {
          ...
          reject(
            new Error(`Git Clone fehlgeschlagen: ${stderr || error.message}`)
          );
          return;
        }
  ...
  async testGitBackupConnection() {
    ...
    try {
      ...
      const commands = [
        ...
        `cd "${
          this.gitBackupPath
        }" && git commit -m "Test connection ${new Date().toISOString()}"`,
        ...
        await new Promise((resolve, reject) => {
          exec(command, { timeout: 30000 }, (error, stdout, stderr) => {
            if (error) {
              reject(
                new Error(
                  `Git Befehl fehlgeschlagen: ${stderr || error.message}`
                )
              );
              return;
            }
  ...
  async pushBackupToGit(backupFile) {
    ...
    try {
      ...
      if (!fs.existsSync(path.join(this.gitBackupPath, ".git"))) {
        console.log(
          "📝 [GIT PUSH] Repository nicht initialisiert, klone neu..."
        );
        await this.cloneGitBackupRepository();
      }
      ...
      const commands = [
        ...
        `cd "${
          this.gitBackupPath
        }" && git commit -m "Add backup: ${backupFileName} - ${new Date().toISOString()}"`,
        `cd "${this.gitBackupPath}" && git push origin ${branch}`,
      ];
      ...
      console.log(
        `✅ [GIT PUSH] Backup erfolgreich gepusht: ${backupFileName}`
      );
      return { success: true, filename: backupFileName };
    } catch (error) {
      console.error(
        `❌ [GIT PUSH] Fehler beim Pushen des Backups:`,
        error.message
      );
  ...
  async createDatabaseBackup(dbConfig, createdBy) {
    ...
    try {
      console.log(
        `📦 [BACKUP] Erstelle ${type} Backup für ${database}@${host}:${port}`
      );
      ...
      console.log(
        `✅ [BACKUP] Backup erstellt: ${filename} (${fileSizeInMB} MB)`
      );
      ...
      return {
        ...
        note:
          type === "mongodb"
            ? "MongoDB Backups werden nicht zu Git gepusht (Verzeichnis)"
            : null,
      };
    } catch (error) {
      console.error(`❌ [BACKUP] Backup-Erstellung fehlgeschlagen:`, error);
  ...
  async createMySQLBackup(dbConfig, filepath) {
    ...
    return new Promise((resolve, reject) => {
      const command = `mysqldump -h ${host} -P ${
        port || 3306
      ...
      exec(
        ...
        (error, stdout, stderr) => {
          if (error) {
            reject(
              new Error(`MySQL Dump fehlgeschlagen: ${stderr || error.message}`)
            );
            return;
          }
          ...
          try {
            ...
            reject(
              new Error(
                `Fehler beim Schreiben der Backup-Datei: ${writeError.message}`
              )
            );
          }
        }
  ...
  async createPostgreSQLBackup(dbConfig, filepath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const command = `pg_dump -h ${host} -p ${
        port || 5432
      } -U ${username} -d ${database} --no-password`;
      ...
      exec(
        ...
        (error, stdout, stderr) => {
          if (error) {
            reject(
              new Error(
                `PostgreSQL Dump fehlgeschlagen: ${stderr || error.message}`
              )
            );
            return;
          }
    ...
    return new Promise((resolve, reject) => {
      const authPart =
        username && password
          ...
          : "";
      const command = `mongodump --host ${host}:${
        port || 27017
      } --db ${database} ${authPart} --out ${backupDir}`;

      exec(command, (error, stdout, stderr) => {
        if (error) {
          reject(
            new Error(`MongoDB Dump fehlgeschlagen: ${stderr || error.message}`)
          );
          return;
        }
    ...
    try {
      ...
      const files = fs
        ...
        .filter((file) => {
          ...
          const stats = fs.statSync(filePath);
          return (
            stats.isFile() && !file.startsWith(".") && file !== "schedules.json"
          );
        })
        .map((file) => {
          const filePath = path.join(backupsPath, file);
      ...
      if (files.length > maxBackups) {
        ...
        console.log(
          `🧹 [CLEANUP] ${filesToDelete.length} alte Backups bereinigt`
        );
      }
    } catch (error) {
      console.error(
        "❌ [CLEANUP] Fehler beim Bereinigen alter Backups:",
        error
      );
    }
  }
  ...
  saveSchedulesToFile(schedules) {
    try {
      ...
      console.error(
        "❌ [SCHEDULE] Fehler beim Speichern der Zeitpläne:",
        error
      );
      throw error;
    }
  ...
  addSchedule(schedule) {
    try {
      ...
      console.error(
        "❌ [SCHEDULE] Fehler beim Hinzufügen des Zeitplans:",
        error
      );
      throw error;
    }
  ...
  createCronJob(schedule) {
    try {
        ...
        async () => {
          ...
          try {
            ...
            if (scheduleIndex !== -1) {
              ...
              schedules[scheduleIndex].nextRun = this.getNextCronRun(
                schedule.cronExpression
              );
              this.saveSchedulesToFile(schedules);
            }
            ...
            console.log(
              `✅ [CRON] Geplantes Backup erfolgreich: ${result.filename}`
            );
          } catch (error) {
            console.error(
              `❌ [CRON] Geplantes Backup fehlgeschlagen für ${schedule.name}:`,
              error
            );
          }
        },
      ...
      console.log(
        `⏰ [CRON] Cron-Job erstellt für: ${schedule.name} (${schedule.cronExpression})`
      );
    } catch (error) {
      console.error(
        `❌ [CRON] Fehler beim Erstellen des Cron-Jobs für ${schedule.name}:`,
        error
      );
      throw error;
    }
  ...
  async checkForUpdates() {
    ...
    try {
      ...
      if (currentRemote !== this.updateRepository) {
        console.log(
          `🔧 [UPDATE] Korrigiere Remote URL: ${currentRemote} → ${this.updateRepository}`
        );
      ...
      const remoteCommit = execSync(
        `git rev-parse origin/${this.updateBranch}`,
        { encoding: "utf8" }
      ).trim();
      ...
      execSync(`git reset --hard origin/${this.updateBranch}`, {
        stdio: "inherit",
      });
  ...
  async backupImportantFiles() {
    ...
    try {
      ...
      console.log(
        `💾 [BACKUP] ${backedUpFiles.length} Dateien gesichert in: ${backupDir}`
      );
    } catch (error) {
      console.error(
        "❌ [BACKUP] Fehler beim Sichern wichtiger Dateien:",
        error
      );
      throw error;
    }
  ...
  async restoreImportantFiles() {
    ...
    try {
      ...
      if (
        fs.existsSync(backupsBackupDir) &&
      ...
      console.log(
        `🔄 [RESTORE] ${restoredFiles.length} Dateien wiederhergestellt`
      );
      ...
      setTimeout(() => {
        try {
          ...
          console.log(
            `🧹 [CLEANUP] Temporäres Backup bereinigt: ${this.currentBackupDir}`
          );
        } catch (cleanupError) {
          console.warn(
            `⚠️ [CLEANUP] Konnte temporäres Backup nicht bereinigen: ${cleanupError.message}`
          );
        }
      }, 5000);
    } catch (error) {
      console.error(
        "❌ [RESTORE] Fehler beim Wiederherstellen wichtiger Dateien:",
        error
      );
      throw error;
    }
  ...
  fixPermissions() {
    try {
      ...
      // Executable files
      const executableFiles = ["update.sh", "ssl-setup.sh", "server.js"];
      ...
      // Configuration files
      const configFiles = ["config.json", "package.json"];
      ...
      console.warn(
        "⚠️ [PERMISSIONS] Konnte nicht alle Berechtigungen reparieren:",
        error.message
      );
    }
  }
  ...
  startSecurityCleanupTasks() {
    ...
    setInterval(() => {
      ...
      if (cleaned > 0) {
        console.log(
          `🧹 [SECURITY] ${cleaned} abgelaufene Failed Attempts bereinigt`
        );
      }
    }, 10 * 60 * 1000);
    ...
    setInterval(() => {
      ...
      if (cleaned > 0) {
        console.log(
          `🧹 [SECURITY] ${cleaned} abgelaufene CAPTCHA Sessions bereinigt`
        );
      }
    }, 3 * 60 * 1000);
    ...
    setInterval(() => {
      ...
      if (cleaned > 0) {
        console.log(
          `🧹 [SECURITY] ${cleaned} abgelaufene Rate Limits bereinigt`
        );
      }
    }, 2 * 60 * 1000);
  ...
  startServer() {
    ...
    if (this.securityConfig.requireHttps) {
      const keyPath =
        this.sslManager?.keyPath || path.join(__dirname, "ssl", "privkey.pem");
      const certPath =
        this.sslManager?.certPath ||
      ...
      if (fs.existsSync(keyPath) && fs.existsSync(certPath)) {
        try {
          ...
          httpsServer.listen(httpsPort, host, () => {
            console.log(
              `🔐 HTTPS Server läuft auf https://${host}:${httpsPort}`
            );
          });
          ...
          httpServer.listen(port, host, () => {
            console.log(
              `🔄 HTTP Redirect Server läuft auf http://${host}:${port} -> HTTPS`
            );
          });
          ...
          console.error(
            "❌ [SSL] HTTPS Server konnte nicht gestartet werden:",
            error.message
          );
    ...
    setInterval(async () => {
      try {
        ...
        if (certCheck.expiresIn <= 7) {
          console.warn(
            `⚠️ [SSL] Zertifikat läuft in ${certCheck.expiresIn} Tagen ab!`
          );
        }
      } catch (error) {
        console.error(
          "❌ [SSL] Fehler bei der Zertifikat-Überwachung:",
          error.message
        );
      }
  ...
  displayStartupInfo() {
    ...
    if (this.securityConfig.requireHttps) {
      ...
      console.log(
        "🔒 SSL Domain: " +
          (this.sslManager?.domain || this.config.ssl?.domain || "localhost")
      );
      console.log(
        "🔑 SSL Methode: " +
          (this.sslManager?.method || this.config.ssl?.method || "selfsigned")
      );
  ...
  async displaySSLStatus() {
    try {
      ...
      if (status.enabled) {
        ...
        console.log(
          `🔄 Auto-Renewal: ${
            status.autoRenewal ? "✅ Aktiviert" : "❌ Deaktiviert"
          }`
        );
        ...
        if (status.alternativeNames && status.alternativeNames.length > 0) {
          console.log(
            `📝 Alternative Names: ${status.alternativeNames.join(", ")}`
          );
        }

        if (status.issueDate) {
          console.log(
            `📅 Ausgestellt am: ${new Date(status.issueDate).toLocaleDateString(
              "de-DE"
            )}`
          );
        }
  ...
  displaySecurityStatus() {
    ...
    console.log(
      `🔐 HTTPS: ${
        this.securityConfig.requireHttps ? "✅ Aktiviert" : "❌ Deaktiviert"
      }`
    );
    console.log(
      `🔑 2FA: ${
        this.securityConfig.enable2FA ? "✅ Aktiviert" : "❌ Deaktiviert"
      }`
    );
    console.log(
      `🛡️ Starke Passwörter: ${
        this.securityConfig.strongPasswords ? "✅ Aktiviert" : "❌ Deaktiviert"
      }`
    );
    console.log(
      `🤖 CAPTCHA: ✅ Aktiviert (nach ${this.securityConfig.captchaThreshold} Fehlversuchen)`
    );
    console.log(
      `🔒 Account-Sperrung: ✅ Aktiviert (nach ${this.securityConfig.maxFailedAttempts} Fehlversuchen)`
    );
    console.log(
      `⏱️ Session-Timeout: ${
        this.securityConfig.sessionTimeout / 1000 / 60
  ...
  async init() {
    ...
    try {
      ...
      // ====== CONFIG-ÜBERSCHREIBUNG VERHINDERN ======
      const originalRequireHttps = this.config.security.requireHttps;
      console.log(`🔐 [INIT] Original requireHttps: ${originalRequireHttps}`);

      // Stelle sicher, dass Umgebungsvariablen Vorrang haben
      if (process.env.REQUIRE_HTTPS !== undefined) {
        this.config.security.requireHttps =
          process.env.REQUIRE_HTTPS === "true";
        console.log(
          `🔐 [INIT] Forciere requireHttps auf: ${this.config.security.requireHttps} (aus ENV: ${process.env.REQUIRE_HTTPS})`
        );
      }

      // Gleiches für andere kritische Boolean-Werte
      if (process.env.ENABLE_2FA !== undefined) {
        this.config.security.enable2FA = process.env.ENABLE_2FA === "true";
        console.log(
          `🔐 [INIT] Forciere enable2FA auf: ${this.config.security.enable2FA} (aus ENV: ${process.env.ENABLE_2FA})`
        );
      }

      if (process.env.SSL_SETUP_ON_START !== undefined) {
        this.config.ssl = this.config.ssl || {};
        this.config.ssl.setupOnStart =
          process.env.SSL_SETUP_ON_START === "true";
        console.log(
          `🔐 [INIT] Forciere SSL setupOnStart auf: ${this.config.ssl.setupOnStart} (aus ENV: ${process.env.SSL_SETUP_ON_START})`
        );
      }

      // SecurityConfig sofort aktualisieren
      this.securityConfig.requireHttps = this.config.security.requireHttps;
      console.log(
        `🔐 [INIT] SecurityConfig aktualisiert: requireHttps = ${this.securityConfig.requireHttps}`
      );

      console.log("[INIT STEP 1/9] Konfiguration geladen und forciert.");

      // Rest der init() Funktion...
      ...
      if (savedToken && this.config.gitBackup) {
        ...
        console.log(
          `✅ [INIT] Git Token geladen (${savedToken.length} Zeichen)`
        );
      } else {
      ...
      console.error(
        "❌ [FATAL INIT ERROR] Kritischer Fehler während der Initialisierung:",
        error
      );
      process.exit(1);
    }
  ...
  preventConfigOverwrite() {
    // Originale Werte sichern
    const originalValues = {
      requireHttps: this.config.security.requireHttps,
      enable2FA: this.config.security.enable2FA,
      strongPasswords: this.config.security.strongPasswords,
      sslSetupOnStart: this.config.ssl?.setupOnStart,
      sslAutoRenewal: this.config.ssl?.autoRenewal,
    };

    // Überwachung einrichten
    Object.defineProperty(this.config.security, "requireHttps", {
      get: function () {
        return originalValues.requireHttps;
      },
      set: function (value) {
        console.log(
          `⚠️ [CONFIG GUARD] Versuch requireHttps zu ändern: ${originalValues.requireHttps} -> ${value}`
        );
        if (process.env.REQUIRE_HTTPS !== undefined) {
          const envValue = process.env.REQUIRE_HTTPS === "true";
          console.log(
            `🔒 [CONFIG GUARD] Blockiert - ENV-Variable hat Vorrang: ${envValue}`
          );
          originalValues.requireHttps = envValue;
        } else {
          originalValues.requireHttps = value;
        }
      },
      configurable: true,
    });

    console.log("🔒 [CONFIG GUARD] Config-Überschreibungsschutz aktiviert");
  }

  // ====== 3. ENVIRONMENT VARIABLES DEBUGGING ======
  debugEnvironmentVariables() {
    console.log("🔧 [ENV DEBUG] Aktuelle Umgebungsvariablen:");
    ...
    relevantEnvVars.forEach((envVar) => {
      ...
      console.log(`   ${envVar}: "${value}" (Type: ${typeof value})`);
...
process.on("unhandledRejection", (reason, promise) => {
  ...
  console.log(
    "⚠️  Secure Anwendung läuft weiter, aber dies sollte behoben werden!"
  );
});
